...
export default function ChapterPanel({ chapterId, onClose }: ChapterPanelProps) {
  ...
  // Si chapterId est null, content sera undefined
  const content = chapterId ? chaptersContent[chapterId] : null;

  // IMPORTANT : Si pas de contenu trouvé (ou pas d'ID), on affiche un message d'attente
  if (!content && chapterId) {
     return <div className="p-10 text-white">Erreur: Contenu introuvable pour {chapterId}</div>;
  }

  // Si aucun chapitre n'est sélectionné (état initial), on retourne null (rien)
